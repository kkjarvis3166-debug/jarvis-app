...
const String kAppVersion = 'V62.0.0';
...
class _ResearchPageState extends State<ResearchPage> {
  ...
  String _campaignText = "買取キャンペーン実施中！";
  DateTime _campaignEndTime = DateTime.now();

  // 査定ロジック用
  int _selectedBaseRate = 50; // 基本買取率
  int _selectedBonusRate = 0; // 買取アップ率 (プルダウン: 0, 5, 10...30)

  int _basePrice = 0;    // 基本査定額 (キャンペーン前)
  int _bonusAmount = 0;  // アップ金額
  int _totalPayout = 0;  // お客様提示合計額 (税込)
  int _totalPayoutEx = 0; // お客様提示合計額 (税抜)
  ...
  Future<void> _checkVersionAndAutoReload() async {
    try {
      ...
      debugPrint('Version check bypassed');
    }
  }
  ...
  Future<void> _loadSettings() async {
    ...
    setState(() {
      ...
      _campaignText = prefs.getString('cp_text') ?? "買取キャンペーン実施中！";
      final endStr = prefs.getString('cp_end');
      if (endStr != null) _campaignEndTime = DateTime.parse(endStr);
    });
    _calc();
  }
  ...
  Future<void> _saveSettings() async {
    ...
    await prefs.setString('cp_text', _campaignText);
    await prefs.setString('cp_end', _campaignEndTime.toIso8601String());
    _calc();
  }

  bool get _isBannerVisible => _isCampaignOn && DateTime.now().isBefore(_campaignEndTime);
  ...
  void _taxCalc(String v, bool isInc) {
    ...
    if (p == 0) {
      isInc ? _exTaxController.clear() : _incTaxController.clear();
      return;
    }
    setState(() {
      if (isInc) {
  ...
  void _calc() {
    ...
    setState(() {
      if (s > 0) {
        double rawBase = (s * _selectedBaseRate / 100) - r;
        _basePrice = max(0, rawBase.floor());
        _bonusAmount = (_basePrice * _selectedBonusRate / 100).floor();
        _totalPayout = _basePrice + _bonusAmount;
        _totalPayoutEx = (_totalPayout / 1.1).floor();
        _sProf = s - _totalPayout;
        _sRate = (s > 0) ? (_sProf / s) * 100 : 0;
        _aFee = (s * 0.1).floor();
        _aProf = s - _aFee - 800 - _totalPayout;
        _aRate = (s > 0) ? (_aProf / s) * 100 : 0;
      } else {
        _basePrice = 0; _bonusAmount = 0; _totalPayout = 0; _totalPayoutEx = 0;
        _sProf = 0; _sRate = 0; _aProf = 0; _aRate = 0; _aFee = 0;
      }
    });
  }
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setModalState) => Padding(
          ...
          child: Column(
            ...
            children: [
              const Text('バナー告知設定', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
              ...
              TextField(
                decoration: const InputDecoration(labelText: '告知内容 (例: リール買取20%UP中！)'),
                onChanged: (v) => _campaignText = v,
                controller: TextEditingController(text: _campaignText),
              ),
              ...
              const SizedBox(height: 10),
              ElevatedButton(onPressed: () { _saveSettings(); Navigator.pop(context); }, child: const Text('設定を保存して戻る')),
              const SizedBox(height: 20),
            ],
          ),
        ),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    SizedBox(width: 85, child: DropdownButtonFormField<int>(value: _selectedBaseRate, items: const [30, 35, 40, 45, 50, 55, 60, 65, 70].map((v) => DropdownMenuItem(value: v, child: Text('$v%'))).toList(), onChanged: (v) { setState(() => _selectedBaseRate = v!); _calc(); }, decoration: const InputDecoration(isDense: true, labelText: '基本率', border: OutlineInputBorder()))),
                  ],
                ),
                const SizedBox(height: 10),
  ...
  Widget _resCard() => Container(
    ...
    padding: const EdgeInsets.all(12),
    decoration: BoxDecoration(color: const Color(0xFFFFF3E0), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFFE65100), width: 4)),
    child: Column(
      children: [
        const Text('お客様提示合計額 (税込)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
        Text('¥ ${_fmt(_totalPayout)}', style: const TextStyle(fontSize: 48, fontWeight: FontWeight.bold, color: Color(0xFFBF360C), height: 1.1)),
        Text('(税抜: ¥ ${_fmt(_totalPayoutEx)})', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.grey)),
        const Divider(color: Color(0xFFE65100), thickness: 1),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceAround,
          children: [
            Column(children: [const Text('基本査定額', style: TextStyle(fontSize: 12)), Text('¥ ${_fmt(_basePrice)}', style: const TextStyle(fontWeight: FontWeight.bold))]),
            const Icon(Icons.add, size: 16),
            Column(children: [Text('アップ分 ($_selectedBonusRate%)', style: const TextStyle(fontSize: 12)), Text('¥ ${_fmt(_bonusAmount)}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.red))]),
          ],
        )
      ],
    ),
...
class NeonBanner extends StatefulWidget {
  ...
  const NeonBanner({super.key, required this.text, required this.endTime});
  @override
      ...
      child: Stack(
        ...
        children: [
          ...
          Column(
            ...
            children: [
              Text(widget.text, style: const TextStyle(color: Colors.red, fontWeight: FontWeight.w900, fontSize: 18), textAlign: TextAlign.center),
              const SizedBox(height: 4),
              Text('〜 ${widget.endTime.month}/${widget.endTime.day} ${widget.endTime.hour}:${widget.endTime.minute.toString().padLeft(2, '0')} まで', style: const TextStyle(color: Colors.red, fontSize: 16, fontWeight: FontWeight.bold)),
            ],
          ),
        ],
      ),
...
class SecureMathPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double d = 0; d < perimeter; d += spacing) {
      Offset pos;
      if (d < w) { pos = Offset(d, 0); }
      else if (d < w + h) { pos = Offset(w, d - w); }
      else if (d < 2 * w + h) { pos = Offset(w - (d - (w + h)), h); }
      else { pos = Offset(0, h - (d - (2 * w + h))); }
